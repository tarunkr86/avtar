...
<div class="container">
	<ul class="breadcrumb">
		...
		<li><a href="#">Email Template</a> <span class="divider">/</span></li>
	</ul>
</div>

<div class="row-fluid" ng-controller="emailTemplatesController">
	<div class="span10">
		<h4>
			<!--  <div ng-class="{'text-center': displayCreateEmailTemplateButton == false, 'none': displayCreateEmailTemplateButton == false}">
            <br/>-->
			<a href="#addEmailTemplatesModal" role="button"
				ng-click="resetEmailTemplate();"
				title="<spring:message code='create'/>&nbsp;<spring:message code='emailTemplate'/>"
	...
	<div class="span2">
		<h4>
			<p class="text-center">
				<spring:message code='emailTemplates.header' />
				&nbsp; <a href="#searchEmailTemplatesModal"
					id="emailTemplatesHeaderButton" role="button"
					ng-class="{'': displaySearchButton == true, 'none': displaySearchButton == false}"
					title="<spring:message code="search"/>&nbsp;<spring:message code="emailTemplate"/>"
					class="btn btn-inverse" data-toggle="modal"> <i
					class="icon-search"></i>
				</a>
			</p>
...
<div class="row-fluid" ng-controller="emailTemplatesController">
	...
	<div>
		<div id="loadingModal" class="modal hide fade in centering"
			role="dialog" aria-labelledby="deleteEmailTemplatesModalLabel"
			aria-hidden="true">
		...
		<div
			...
			<h4>
				<i class="icon-info-sign"></i>
				<spring:message code="emailTemplates.emptyData" />
			</h4>
			<br />

			<p>
				<spring:message code="emailTemplates.emptyData.text" />
			</p>
		</div>
		...
		<div id="gridContainer"
			...
			<table class="table table-bordered table-striped">
				<thead>
					<tr>
						<th scope="col"><spring:message code="emailTemplates.name" /></th>
						<th scope="col"><spring:message code="emailTemplates.subject" /></th>
						<th scope="col"><spring:message code="emailTemplates.message" /></th>
						<th scope="col"><spring:message code="emailTemplates.category" /></th>																					
						<th scope="col"><spring:message	code="emailTemplates.action" /></th>
					</tr>
				</thead>
				<tbody>
					<tr ng-repeat="emailTemplate in page.source">
                        
						<td class="tdEmailTemplatesCentered">{{emailTemplate.name}}</td>
						<td class="tdEmailTemplatesCentered">{{emailTemplate.subject}}</td>
						<td class="tdEmailTemplatesCentered">{{emailTemplate.message}}</td>
						<td class="tdEmailTemplatesCentered">{{emailTemplate.category.name}}</td>																														
						<td class="width15">
							<div class="text-center">
								<input type="hidden" value="{{emailTemplate.id}}" /> <a
									href="#updateEmailTemplatesModal"
									ng-click="selectedEmailTemplate(emailTemplate);" role="button"
									title="<spring:message code="update"/>&nbsp;<spring:message code="emailTemplate"/>"
									class="btn btn-inverse" data-toggle="modal"> <i
									class="icon-edit"></i>
								</a>
								 <a href="#deleteEmailTemplatesModal"
                               ng-click="selectedEmailTemplate(emailTemplate);"
                               role="button"
                               title="<spring:message code="delete"/>&nbsp;<spring:message code="emailTemplate"/>"
		...
		<jsp:include page="dialogs/emailTemplatesDialogs.jsp" />

	</div>
</div>

<script src="<c:url value="/resources/js/pages/emailTemplates.js" />"></script>